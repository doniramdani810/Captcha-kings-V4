...
access_token = "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"
...
# Menutup program "Captchakings.exe"
for proc in psutil.process_iter(['name']):
    if proc.info['name'] == 'Captchakings.exe':
        proc.kill()
    ...
    premium_levels = {
        "Tingkat Prajurit": {
            ...
            "fitur": [
                ...
                "Ping Internet",
                "Reset Internet Auto",
                "Hanya bisa menggunakan 8 akun sandboxie"
            ]
        },
        "Tingkat Panglima": {
...
def def_registrasi():
    ...
    info_button = Button(register_window, text="Baca ini untuk melihat apa saja yang didapat di setiap tingkatan!", fg="black", font=("Arial", 8), padx=10, command=show_detail)
...
def check_login(login_id_entry, login_password_entry):
    ...
    def save_user_info(username, password, mac_address, remember_me):
        # membuat file teks untuk menyimpan username, password, dan MAC Address
...
def show_account_window_prajurit(root, id, tingkatan, exp_date):
    ...
    # atur ukuran jendela dan posisinya
    ...
    def edit_config(api_key):
        ...
        with open(r"C:\Users\Administrator\Desktop\V4\tem.txt", "r") as f:
            config_data = json.load(f)
            config_data["ApiKey"] = api_key
        url = "https://raw.githubusercontent.com/doniramdani810/Captcha-kings-V4/main/USERAGENT.txt"
        response = urllib.request.urlopen(url)
        user_agents = response.read().decode().split('\n')
        user_agent = random.choice(user_agents).strip()
        config_data["Window"]["UserAgent"] = user_agent
        with open(r"C:\Users\Administrator\Desktop\V4\tem.txt", "w") as f:
        ...
        if account_window.winfo_exists():
            if os.name == "nt":
                subprocess.Popen(["start", "/HIGH", "Captchakings.exe"], shell=True, stdin=subprocess.DEVNULL, bufsize=0, cwd=r"C:\Users\Administrator\Desktop\V4")
            elif os.name == "posix":
                subprocess.Popen(["/bin/sh", "-c", "wine Captchakings.exe &"], stdin=subprocess.DEVNULL, bufsize=0, cwd=r"C:\Users\Administrator\Desktop\V4")
            else:
                messagebox.showerror("Unsupported OS", "Your operating system is not supported.")
            time.sleep(10)
        else:
            # kill all running processes with the name "Captchakings.exe"
            for proc in psutil.process_iter():
                if proc.name() == "Captchakings.exe":
                    proc.kill()
    ...
    def do_action():
        ...
        with open(r"C:\Users\Administrator\Desktop\V4\key.txt", "r") as key_file:
            keys = key_file.readlines()
        for api_key in keys[:selected_option]:
            edit_config(api_key.strip())
